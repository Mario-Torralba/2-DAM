...
For Microsoft Windows, download WoPeD-install-windows-3.7.1.zip, 
unpack it with an archiving tool and run the contained executable 
WoPeD-install.exe.  

For Linux, download WoPeD-install-linux-3.7.1.tgz, unpack and 
decompress it with the tar utility. Then launch the contained executable 
WoPeD-install-linux.jar. 

For MacOS, download WoPeD-install-macos-3.7.1.dmg, double-click on it
...
Root directory:
  ...
  ./WoPeD-classes-3.7.1.jar         (WoPeD program classes)
